...
;; Authors: Kajetan Rzepecki and Michael Dickens
...
(defcustom gamify-level-names
  '("Uninitiated"
    ...
    "Supreme Legend"
    "Eternal")
  "A list of gamify level names."
  :group 'gamify)
    

(defcustom gamify-stat-levels
  (let ((res nil) (exp 0) (adder 0))
    (dolist (name gamify-level-names)
      (setq exp (+ exp adder))
      (setq adder (+ 80 adder))
      (push (cons exp name) res))
    (nreverse res))
  "An alist of Gamify levels and their exp values."
  :type 'boolean
  :group 'gamify)

(defcustom gamify-stat-levels-old
  '((0 . "Dabbling")
    (500 . "Novice")
    (1100 . "Adequate")
    (1800 . "Competent")
...
(defvar gamify-dot-level-colors gamify-stat-levels)
(defvar gamify-dot-level-colors-old
  '(("Dabbling")
    ("Novice")
    ("Adequate")
    ("Competent")
...
- Tag your tasks with something meaningful, e. g. \"coding\".
